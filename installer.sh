...
exec > >(tee -a /tmp/Filmon_debug.txt) 2>&1
set -x
...
wget --no-check-certificate 'https://github.com/Belfagor2005/Filmon/archive/refs/heads/main.tar.gz'
tar -xzf main.tar.gz
cp -r 'Filmon-main/usr' '/'
set +e
cd
sleep 2
...
IMAGE VERSION: $distro_version" >> /tmp/Filmon_debug.txt
sleep 5
killall -9 enigma2
exit 0